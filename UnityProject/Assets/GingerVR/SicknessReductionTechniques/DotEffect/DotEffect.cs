...
public class DotEffect : MonoBehaviour
{
    ...
    [Range(1, 20), Tooltip("Radius around player in which dots won't be shown")]
    public float playerRadius = 3f;
    [Tooltip("If true, dots will also follow player rotation")]
    ...
    void Update()
    {
        ...
        {
            if (Vector3.Distance(subject.gameObject.transform.position, dot.gameObject.transform.position) <= playerRadius)
            {
                dot.gameObject.GetComponent<MeshRenderer>().enabled = false;
            }
            else
                dot.gameObject.GetComponent<MeshRenderer>().enabled = true;
        }
        if(followRotation)
